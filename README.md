...
- **STRUCTARCH_DAG_ResolveCSR**: Dense `DagNodeID` (`uint32`) graph in CSR form (`DagNodeCSR` rows + flat `edges` array). Outputs a flat `ExecutionOrder` and `PhaseOffsets` via `memarch`/`memstruct` marks—no hash maps or jagged slices. Intended for frame graphs with a fixed pass cap and manual-memory clients.
